  ...
  <div class=" tw-mx-4 tw-bg-white tw-shadow tw-rounded-xl tw-px-4 tw-py-4">
    ...
    <div class="resumo-pagamento-card">
      ...
      <div class="resumo-bloco" v-for="(agrup, idx) in order.passagens_agrupadas":key="idx">
        ...
        <div class="resumo-linha">
          <template v-if="temRede(agrup.passagem_pedidos)">
            <!-- Ícone de rede (imagem baiano.png) -->
            <img
              src="@/assets/images/baiano.png"
              alt="Rede"
              style="width: 24px; height: 24px; object-fit: contain; filter: invert(27%) sepia(7%) saturate(484%) hue-rotate(175deg) brightness(40%) contrast(120%);"
            />
          </template>
          <template v-else>
            <!-- Ícone padrão (cama) -->
...
function temRede(passagem_pedidos) {
  return passagem_pedidos.some(it =>
    typeof it.comodo.tipo_comodidade === 'string'
      ? it.comodo.tipo_comodidade.toLowerCase() === 'rede'
      : it.comodo.tipo_comodidade?.nome?.toLowerCase() === 'rede'
  );
}
